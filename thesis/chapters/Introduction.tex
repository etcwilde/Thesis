...
is desires, it is usually used in the software development process.
...
called the \mt{}, is constructed from the underlying graph of the
repository.
Compared to the graph, trees are relatively easy to
visualize, and there are many visualization metaphors that take
advantage of different properties of trees.
The \mt{} abstracts the repository commit graph into a set of trees,
each rooted at a merge into the master branch of the repository.
The leaves of the tree are the commits, and the inner nodes of the tree
represent the merges leading to the integration of the commits.
Second, this \paper{} proposes several visualizations that take advantage
of the \mt{} model.
Third, an implementation of the visualizations in a tool called \tool{}.
Finally, an evaluation of the tool to determine it's usability.
This \paper{} specifically studies the Linux repository.
The Linux repository is large and complex, with more than 1000
active contributors.

The central goal of this \paper{} is to improve visualizations of
git repositories in a way that provides meaningful insight into how
commits are integrated into a repository.

\begin{textbox}
  \textbf{Overarching Question:} How do we effectively visualize the
...
including it's search engine, summarization tables, and tree visualizations.
