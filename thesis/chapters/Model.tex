...
We must assure that the trees we created using the algorithm are an
accurate representation of the events occurring in the repository.
Evaluation poses some issues, as there is no easy way to accurately
gather this information directly from the DAG\@. Further inspection of
the merges provides some insight; Linus Torvalds adds useful information
about the content of the merges. For each branch being merged, Linus
includes either the first 20 commit titles and the total number of
commits being merged (see Figure~\ref{fig:sampleMerge} for an example),
or the list of titles if 20 or fewer commits are being merged from the
branch.
...
Using this information, we are able to verify that the results from our
database match the information in the merge logs, and further
investigate the merges that don't. If the merge-tree in
our database has the correct number of commits being merged, and the
titles listed in the log appear in the merge-tree, then we have
correctly identified and recorded the commits and merge-tree.
We use a scripted approach to extract information from both the git logs
and our database. From the logs, we extract the commit hash, the date
the merge was created, and the number of commits merged.

We gather the information from the logs using two commands;\\
...
Using this technique, we collected 15096 merges by Linus from our
database, and 15306 merges from the logs. There was an overlap of 14836
merges between the two, with 470 merges from the logs that were not in
the database, and 260 merges that were in the database that were not in
the logs, or a total of 15566 merges.
...
they were collected. From the logs, we collect all information for
merges made by Linus since the merge tag \emph{v3.16}, regardless of
whether they are in the master branch or not. We annotate the commits
using the list of commits that are along the first-parent starting at
the merge tag, which we know is on the master branch. Merges that are
not on the master branch are not removed from the dataset. The merges
collected for the database were ones we were able to identify along the
master branch. Focusing only on the merges in the master branch, the
difference between the two sets decreases dramatically. There were 14670
merges along the master branch that were shared by both the logs and by
our database, 2 merges, \emph{186051d70444742bf1c2bc0257dd4696a3df66e3}
and \emph{5170a3b24a9141e2349a3420448743b7c68f2223}, from the logs that
were not in the database, and 426 merges in the database that were not
in the logs. Examining the two merges from the logs, we find that the
master branch disappears into a group of commits, which are later
merged. This is confusing, as Linus is the only one with direct merge
access into the master branch of the repository. The likely reason for
this is that these commits were merged in a fast-forward merge, which
does not insert a merge, and instead splices the commits directly into
the branch.
...
we need the merges to show up in both datasets, and that they have
associated commit logs in both our database and in the merge log. There
are 14198 merges with these properties.
...
\begin{itemize}
  \item
    ...
    and the logs. Upon further investigation, we found that four merges
    had incorrectly formatted logs. The fifth merge,
    \emph{42a579a0f960081cd16fc945036e4780c3ad3202}, is a \foxtrot{}
    merge. One commit is on the first-parent of the merge, and is
    therefore not detected when building the merge-tree, but is included
    in the merge log.
  ...
  \item
    ...
    foxtrot occurred. Upon further investigation, we find that Dave
    Jones merges 29 commits from ranging from October 15, 2006 to
    December 12, 2006 into the master branch. Due to the \foxtrot{},
    this merges 4708 repository events, including 199 merges by Linus
    Torvalds.
