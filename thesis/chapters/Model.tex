...
We must assure that the trees we are creating with the algorithm are an
accurate representation of the events occurring in the repository.
Evaluation poses some issues, as there is no easy way to gather a ground
truth directly from the DAG\@. Further inspection of the integrating
merges show that the merge log made by Linus Torvalds contains useful
information about the contents of that merge, including the number of
commits, and a listing of a subset of the commit log titles (example of
a merge message from the kernel shown in Figure~\ref{fig:sampleMerge}).
...
Using this information, we are able to determine how many commits were
integrated at that merge, and a list of the first 20 non-merge commits
that appear in the merge. As this log information is computer-generated,
we can extract this information with relative ease, and compare the log
information with the contents of the corresponding merge-tree. If the
merge-tree contains the correct number of commits, and the commits
listed in log message are present in the tree, then the commit and tree
were correctly identified.

Since Linus Torvalds primarily merges commits into the master branch, we
make the initial assumption that every merge by Linus is the root of a
merge-tree. In a later pass, we detect merges made by Linus that are to
other branches, and remove them from the set of root merges. We extract
the merges made by Linus Torvalds using the command
\verb|git log --merges --author='Torvalds'| and compared the number of
commits stated in the log message with the number of commits in the
merge-tree. Using the example in Figure~\ref{fig:sampleMerge}, we would
expect to find that the merge-tree rooted at \emph{8cbd84f} contains 24
commits, and the one-line commit titles are found on commits within the
merge-tree. Those that were reported to be incorrect by this method were
further inspected manually to ensure that they were true errors.
